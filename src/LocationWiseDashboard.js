...
const LocationWiseDashboard = () => {
    ...
    const [allLocationYesImageData, setAllLocationYesImageData] = useState({
        labels: [],
        datasets: [
          {
            label: "No. of Images",
    ...
    useEffect(() => {
        ...
        const fetchAllYesGraphImageData = async () => {
            try {
                const imageData = [];
                for (const location of userLog.locations) {
                  const response = await axios.get(`${API_URL}/graph9`, {
                    params: { locationname: `${location.name} District Court` }
                  });
                  imageData.push({
                ...
                setAllLocationYesImageData(imageData);
              } catch (error) {
                console.error("Error fetching data:", error);
              }
          };
