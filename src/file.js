...
const File = () => {
  ...
  const [manpowerForm, setManpowerForm] = useState({  
    ...
    Ready_Cust_QA_MP:'',Cust_QA_Done_MP:'',PDF_Export_MP:'',Refilling_Files_MP:'',Inventory_MP:'',
  })
  ...
  const handleSelectPM = (name) => {
    setSelectedPM(name); 
    showProjectMan(!projectMan);
  };
  const handleSelectSM = (name) => {
    setSelectedSM(name);
    showSiteMan(!siteMan);
  };
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setManpowerForm({ ...manpowerForm, [name]: value });
  }
  ...
  return (
    <>
      ...
      <div className='container '>
        <div className='row'>
          <div className='col-2'></div>
          <div className='col-10 main-updc'>
            ...
            <div className='row mt-5'>
              <div className='col-4'>
                <button className='btn location-btn' onClick={handleShowLocation}>Location</button>
                {showLocation && (
                  <>
                    <div className='locations-card'>
                      {location.map((elem, index) => (
                        <div key={index}><p>{elem.LocationName}</p></div>
                      ))}
                    </div>
                  </>
                )}
              ...
              <div className='col-6'>
                <span>Date: </span><DatePicker className='date-field' selected={startDate} onChange={(date) => setStartDate(date)} />
              </div>
            </div>
            <div className='row mt-4'>
              <div className='col-5'>
                <div className='row'>
                  ...
                  <div className='col-7'><input type='text' placeholder='Select PH Name'  name='PH_Id' onClick={handleShowProjectHead} onChange={handleInputChange}/></div>
                  {/* {projectHead && (
                ...
                <div className='row mt-2'>
                  ...
                  {projectMan && (
                    <div className='dropdown-card'>
                    {usermaster.filter(user => user.designation.toLowerCase() === "project manager").map((elem,index) => (
                      <div  key={index} onClick={() => handleSelectPM(`${elem.first_name} ${elem.last_name}`)}>
                      <p>{elem.first_name} {elem.last_name}</p>
                    </div>
                    ))}
                ...
                <div className='row mt-2'>
                  ...
                  {siteMan && (
                    ...
                    {usermaster.filter(user => user.designation.toLowerCase() === "site manager" || "site incharge").map((elem,index) => (
                      <div  key={index} onClick={() => handleSelectSM(`${elem.first_name} ${elem.last_name}`)}>
                      <p>{elem.first_name} {elem.last_name}</p>
                    </div>
                    ))}
        ...
        <div className='row'>
          ...
          <div className='col-10'>
            <form onSubmit={handleFormSubmit}>
              ...
              {/* <button className='btn process-btn' onClick={handleDeleteDesignation}>Delete</button> */}
            </form>
            <table className='table-bordered'>
              <thead>
                <tr>
